...
os.environ["TRANSFORMERS_CACHE"] = "AI_Model/AI_model_cache/Image2Molecule"
os.environ["HF_DATASETS_CACHE"] = "AI_Model/AI_model_cache/Image2Molecule"
os.environ["HF_METRICS_CACHE"] = "AI_Model/AI_model_cache/Image2Molecule"
def load_mobilenet(pretrained=True, cache_dir='AI_Model/AI_model_cache/Image2Molecule'):
    if cache_dir:
...
class Smiles2Image():
    def __init__(self):
        self.tokenizer=AutoTokenizer.from_pretrained("ibm/MoLFormer-XL-both-10pct", trust_remote_code=True,model_max_length=49, cache_dir="AI_Model/AI_model_cache/Image2Molecule")
        self.model=CNNLSTMModelMobileNet(embedding_dim=300, hidden_dim=512, vocab_size=self.tokenizer.vocab_size+1)
        self.model.load_state_dict(torch.load('AI_Model/AI_model_cache/Image2Molecule/Model (best).pt',weights_only=False))
        self.model.eval()
